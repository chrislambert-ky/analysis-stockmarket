...
def transform_buy_on_dip(df):
    ...
    latest_close = df['Close'].iloc[-1]  # latest market close for valuation
        ...
        for level in dip_levels:
            ...
            if low_price <= target_price:
                purchased_list.append({
                    ...
                    'Current_Value': round(latest_close, 6),  # 1 share × latest close
                    'Return_per_Share': round(latest_close - target_price, 6)
                })
    ...
    bod_df['Cumulative Current Value'] = bod_df['Current_Value'].cumsum()
    bod_df['Cumulative Return'] = bod_df['Cumulative Current Value'] - bod_df['Cumulative Dollars Invested']
