...
def transform_buy_on_dip(df):
    """Simulate Buy-on-Dip strategy (1 share per 1%-5% drop). Adds cumulative shares column."""
    purchased_list = []
        ...
        for level in sorted(dip_levels):  # Ensure buy levels are processed from smallest to largest
            ...
            if low_price <= target_price:
                purchased_list.append({
                    ...
                    'Buy_Level_Float': (1 - level),  # for sorting
                    'Shares Purchased': 1
                })
    ...
    # Sort by Date (oldest to newest), then by Buy_Level (smallest to largest drop)
    df_bod = pd.DataFrame(purchased_list)
    df_bod = df_bod.sort_values(['Date', 'Buy_Level_Float']).reset_index(drop=True)
    df_bod['Cumulative Shares'] = df_bod['Shares Purchased'].cumsum()
    df_bod = df_bod.drop(columns=['Buy_Level_Float'])
    return df_bod
...
for ticker_symbol in etf_list:
    ...
    # Save complete history for each ticker
    if not df.empty:
        csv_name = os.path.join(output_folder, f"{ticker_symbol.lower()}_full_history.csv")
        df.to_csv(csv_name, index=False)
        print(f"Saved full history for {ticker_symbol} → {csv_name}")